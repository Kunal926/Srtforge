...
Set HF\_TOKEN before installing if you need authenticated Hugging Face access.
...
.\\.venv\\Scripts\\Activate.ps1  
.\\scripts\\rebuild\_studio\_sidecar.ps1  
cd .\\srtforge-studio  
pnpm install  
pnpm tauri dev
...
srtforge-studio/src-tauri/binaries/srtforge\_worker/srtforge\_worker.exe, runs

gpu-smoke, and checks that worker \--no-preload reaches worker\_ready
...
* .venv/, node\_modules/, target/, dist/, build/  
